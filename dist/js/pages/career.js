...
function getCareers(table) {
    $("#careers").dataTable().fnDestroy();
    $("#dirac_careers").html("");
        ...
        success: function (response) {
            if (response.errorCode === 0) {
                ...
                $.each(response.data, function (index, value) {
                    ...
                    requests += '<td>'
                            + '<form name="newCareerForm" id="newCareerForm" method="POST" action="new_career.php">'
                            + '<input type="hidden" name="id" value="' + value.id + '" />'
                            + '<button data-toggle="tooltip" title="Editar" type="submit" class="btn btn-primary btn-flat"><li class="fa fa-pencil"></li></button>'
                            + '</form>'
                            + '<button data-toggle="tooltip" title="Eliminar" class="btn btn-primary btn-flat" onClick="deleteRC(\'cat_escolaridad\', ' + value.id + ')"><li class="fa fa-minus-circle text-danger"></li></button></td>';
                });
                $("#dirac_careers").append(requests);
                $("#careers").dataTable({
                    "dom": 'Bfrtip',
                    "buttons": [
